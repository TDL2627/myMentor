...
<div class="container">
  <div class="row"  v-for="ment in mental" :key="ment.title">
    <div class="col-lg-6">
     <h3>{{ment.title}}</h3>
     <p>{{ment.details}}</p>
    </div>
    <div class="col-lg-6" >
       <img class="piks" :src="require('@/assets/'+ ment.pic)">
    </div>
    <hr>
  </div>
</div>
...
async created () {
    this.loading = true
    try {
      const res = await fetch('https://mymentor-server.herokuapp.com/info/mental')
      this.mental = await res.json()
      this.loading = false
    } catch (error) {
      console.log(error)
      this.loading = false
