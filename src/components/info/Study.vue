<template>
  <div class="study">
       <Nav/>
<h1 class="heading animate__animated animate__fadeInDown">STUDY GUIDE</h1>
...
async created () {
    this.loading = true
    try {
      const res = await fetch('https://mymentor-server.herokuapp.com/info/study')
      this.study = await res.json()
      this.loading = false
    } catch (error) {
      console.log(error)
      this.loading = false
